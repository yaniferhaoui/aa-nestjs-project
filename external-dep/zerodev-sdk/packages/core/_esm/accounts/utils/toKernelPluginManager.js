...
export async function toKernelPluginManager(client, { sudo, regular, pluginEnableSignature, validatorInitData, action, validAfter = 0, validUntil = 0, entryPoint: entryPointAddress, kernelVersion }) {
    ...
    const getPluginEnableSignature = async (accountAddress) => {
        ...
        if (!regular)
            throw new Error("regular validator not set");
        const { version } = await accountMetadata(client, accountAddress, entryPointAddress);
        let ownerSig;
        if (entryPointVersion === "v0.6") {
