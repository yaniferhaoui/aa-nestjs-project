import type { ENTRYPOINT_ADDRESS_V06_TYPE, EntryPoint } from "permissionless/types";
import type { GetEntryPointVersion } from "permissionless/types/entrypoint";
import type { UserOperation, UserOperationWithBigIntAsHex } from "permissionless/types/userOperation";
...
export type ZeroDevPaymasterRpcSchema<entryPoint extends EntryPoint> = [
    {
        ...
        Parameters: [
            {
                chainId: number;
                userOp: GetEntryPointVersion<entryPoint> extends "v0.6" ? PartialBy<UserOperationWithBigIntAsHex<"v0.6">, "callGasLimit" | "preVerificationGas" | "verificationGasLimit"> : PartialBy<UserOperationWithBigIntAsHex<"v0.7">, "callGasLimit" | "preVerificationGas" | "verificationGasLimit" | "paymasterVerificationGasLimit" | "paymasterPostOpGasLimit">;
...
export type ValidatorType = Extract<keyof typeof VALIDATOR_TYPE, "PERMISSION" | "SECONDARY">;
export type KernelValidator<entryPoint extends EntryPoint, Name extends string = string> = LocalAccount<Name> & {
    validatorType: ValidatorType;
    getNonceKey: (accountAddress?: Address, customNonceKey?: bigint) => Promise<bigint>;
    getDummySignature(userOperation: UserOperation<GetEntryPointVersion<entryPoint>>, pluginEnableSignature?: Hex): Promise<Hex>;
    signUserOperation: (userOperation: UserOperation<GetEntryPointVersion<entryPoint>>, pluginEnableSignature?: Hex) => Promise<Hex>;
...
export type KernelPluginManager<entryPoint extends EntryPoint> = KernelValidator<entryPoint> & {
...
export type PluginInstallData<entryPoint extends EntryPoint> = entryPoint extends ENTRYPOINT_ADDRESS_V06_TYPE ? {
    selector: Hex;
    executor: Address;
    validator: Address;
    validUntil: number;
