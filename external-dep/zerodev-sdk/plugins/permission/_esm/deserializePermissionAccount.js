import { KernelFactoryStakerAbi, KernelV3AccountAbi, createKernelAccount } from "@zerodev/sdk";
import { toKernelPluginManager } from "@zerodev/sdk/accounts";
import { getEntryPointVersion } from "permissionless";
import { decodeFunctionData } from "viem";
import { privateKeyToAccount } from "viem/accounts";
import { toCallPolicy, toGasPolicy, toRateLimitPolicy, toSignatureCallerPolicy, toSudoPolicy, toTimestampPolicy } from "./policies/index.js";
...
export const deserializePermissionAccount = async (client, entryPointAddress, modularPermissionAccountParams, modularSigner) => {
    const entryPointVersion = getEntryPointVersion(entryPointAddress);
    if (entryPointVersion !== "v0.7") {
        throw new Error("Only EntryPoint 0.7 is supported");
    }
    const params = deserializePermissionAccountParams(modularPermissionAccountParams);
    let signer;
    ...
    const modularPermissionPlugin = await toPermissionValidator(client, {
        signer,
        policies: await Promise.all(params.permissionParams.policies?.map((policy) => createPolicyFromParams(policy)) || []),
