...
import { encodeAbiParameters, getTypesForEIP712Domain, hashTypedData, keccak256, validateTypedData } from "viem";
import { toAccount } from "viem/accounts";
import { signMessage } from "viem/actions";
import { getChainId } from "viem/actions";
import { getValidatorAddress } from "./index.js";
import { b64ToBytes, deserializePasskeyValidatorData, findQuoteIndices, isRIP7212SupportedNetwork, parseAndNormalizeSig, serializePasskeyValidatorData, uint8ArrayToHexString } from "./utils.js";
...
export async function createPasskeyValidator(client, { passkeyName, passkeyServerUrl, entryPoint: entryPointAddress, validatorAddress }) {
    validatorAddress =
    ...
    const authenticatorIdHash = keccak256(uint8ArrayToHexString(b64ToBytes(registerCred.id)));
    // Verify registration
...
export async function getPasskeyValidator(client, { passkeyServerUrl, entryPoint: entryPointAddress, validatorAddress }) {
    validatorAddress =
        validatorAddress ?? getValidatorAddress(entryPointAddress);
    // Get login options
    ...
    // get authenticatorIdHash
    const authenticatorIdHash = keccak256(uint8ArrayToHexString(b64ToBytes(loginCred.id)));
...
export async function deserializePasskeyValidator(client, { serializedData, entryPoint: entryPointAddress }) {
    const { passkeyServerUrl, entryPoint, validatorAddress, pubKeyX, pubKeyY, authenticatorIdHash } = deserializePasskeyValidatorData(serializedData);
    // Fetch chain id
    const chainId = await getChainId(client);
    // build account with passkey
    const account = toAccount({
