...
import { decodeFunctionData, encodeAbiParameters, getAbiItem, hexToSignature, isHex, pad, padHex, signatureToHex, toFunctionSelector, toHex, zeroAddress } from "viem";
...
export const serializeSessionKeyAccountParams = (params) => {
    const jsonString = JSON.stringify(params);
    const uint8Array = new TextEncoder().encode(jsonString);
    const base64String = bytesToBase64(uint8Array);
    return base64String;
};
export const deserializeSessionKeyAccountParams = (params) => {
...
export const findMatchingPermissions = (callData, permissionsList) => {
    try {
        ...
        if (functionName === "execute") {
            ...
            const [to, data] = args;
            return filterPermissions([to], [data], [0n], permissionsList, Operation.DelegateCall)?.[0];
        }
...
const filterPermissions = (targets, dataArray, values, permissionsList, operation = Operation.Call) => {
